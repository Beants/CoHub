...
_NOISE_TOKENS = ("加载中", "loading", "…", "...", "查看更多")


def candidate_summary_is_reliable(candidate: CandidateSummary) -> bool:
    """Check if an extracted candidate summary looks valid."""
    name = (candidate.display_name or "").strip()
    if not name or len(name) > 80:
        return False
    if candidate.candidate_id in ("search", "", "undefined", "null"):
        return False
    if any(noise in name.lower() for noise in _NOISE_TOKENS):
        return False
    return True


def candidate_batch_is_reliable(candidates: list[CandidateSummary]) -> bool:
    """Check if the majority of extracted candidates are reliable."""
    if not candidates:
        return True
    reliable = sum(1 for c in candidates if candidate_summary_is_reliable(c))
    return reliable / len(candidates) >= 0.5
...
async def extract_candidates_from_page(
    ...
    try:
        ...
        logger.warning("Failed to extract candidate cards from page", exc_info=True)
        raw_cards = []
